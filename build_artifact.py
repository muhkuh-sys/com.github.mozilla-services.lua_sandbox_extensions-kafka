...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-kafka-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-kafka-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-kafka-*.xml".')
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-kafka-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-kafka-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-kafka-*.xml".')
